...
def load_data__(
    ...
    data = [[None] * len(names) for _ in range(len(y_names))]
    for d in range(len(y_names)):
        for i in range(len(names)):
            fname = f"{root}/data/{x_name}_{y_names[d]}_{names[i]}.csv"
            data[d][i] = np.loadtxt(fname, delimiter=" ")[:, 1]  # type: ignore
