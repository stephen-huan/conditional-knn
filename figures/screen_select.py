...
if __name__ == "__main__":
    ...
    rng.__setstate__(
        {"bit_generator": "PCG64",
         "state": {"state": 116693921324311315271032145974622854199,
    ...
    train = plt.scatter(x_train, y_train, label=r"$X_{Tr}$ train",
                        s=POINT_SIZE, zorder=2.5, color=colors[0])
    test = plt.scatter(x_test, y_test, label="$X_{Pr}$ predict",
                       s=BIG_POINT, zorder=2.75, color=colors[1])
    mean, = plt.plot(x, mu, label="conditional mean",
                     color=colors[-1], linewidth=LINEWIDTH)
    sigma = plt.fill_between(x.flatten(), mu - 2*std, mu + 2*std,
                             label=r"$2 \sigma$", color=colors[-1], alpha=0.15)
    ...
    for s in range(1, S + 1):
        ...
        train = plt.scatter(x_train, y_train, label="training data",
                            s=POINT_SIZE, zorder=2.5, color=colors[0])
        test = plt.scatter(x_test, y_test, label="testing data",
                        s=BIG_POINT, zorder=2.75, color=colors[1])
        select = plt.scatter(x_train[selected], y_train[selected],
                            label="selected",
                            s=BIG_POINT, zorder=2.5, color=colors[3])
        mean, = plt.plot(x, mu, label="conditional mean",
                         color=colors[-1], linewidth=LINEWIDTH)
        sigma = plt.fill_between(x.flatten(), mu - 2*std, mu + 2*std,
                                label=r"$2 \sigma$",
